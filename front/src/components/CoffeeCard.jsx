...
const CoffeeCard = ({coffee}) => {
    ...
    const handleFavoriteClick = async () => {
      try {
        const response = await axios.put('http://localhost:3000/atualizar-favoritos', {
          name: name
        });
        console.log('Favorito atualizado com sucesso:', response.data);
      } catch (error) {
        console.error('Erro ao atualizar favorito:', error.response ? error.response.data : error.message);
      }
    };
